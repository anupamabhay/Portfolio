...
const categoryColors: Record<string, string> = {
  Frontend: "border-sky-500/30 bg-sky-500/10 text-sky-600 dark:text-sky-300",
  Backend: "border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300",
  "DevOps & Cloud": "border-orange-500/30 bg-orange-500/10 text-orange-600 dark:text-orange-300",
  Tools: "border-purple-500/30 bg-purple-500/10 text-purple-600 dark:text-purple-300",
}
...
export function Skills() {
  ...
  return (
    <section id="skills" className="py-24 px-6 bg-surface-secondary/50" ref={ref}>
      <div className={`max-w-6xl mx-auto transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {Object.entries(skills).map(([category, items]) => (
            <div
              key={category}
              className="p-6 rounded-xl border border-edge bg-surface-tertiary/30"
            >
